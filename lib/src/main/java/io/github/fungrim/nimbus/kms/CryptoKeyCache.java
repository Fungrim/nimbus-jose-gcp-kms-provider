...
public class CryptoKeyCache {
    ...
    public CryptoKeyCache(Duration cacheDuration, KmsServiceClient client, KeyIdGenerator idGenerator) {
        ...
        this.entryCache = CacheBuilder.newBuilder().expireAfterAccess(cacheDuration.toMillis(), TimeUnit.MILLISECONDS)
    ...
    public Optional<Entry> find(JWSAlgorithm alg) {
        ...
        return client.list(k -> Algorithms.getSigningAlgorithmUnchecked(k).equals(alg))
                .max((a, b) -> Keys.extractVersion(a).compareTo(Keys.extractVersion(b))).map(this::keyToEntry);
    }
    ...
    public Stream<Entry> listByAlgorithm(Predicate<JWSAlgorithm> filter) {
        ...
        // unchecked algorithm extract should be OK here since the client
        // already has filtered the keys
        return client.list(k -> filter.test(Algorithms.getSigningAlgorithmUnchecked(k))).map(this::keyToEntry);
    }

    public Stream<Entry> listByKeyVersion(Predicate<CryptoKeyVersion> filter) {
        Preconditions.checkNotNull(filter);
        return client.list(filter).map(this::keyToEntry);
    }

    private Entry keyToEntry(CryptoKeyVersion v) {
        Entry e = new Entry(v, Keys.parseVersionName(v.getName()), idGenerator.getKeyId(v),
                Algorithms.getSigningAlgorithmUnchecked(v));
